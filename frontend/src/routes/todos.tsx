...
import { useTodos, useToggleTodo, useAddTodo, useRecipients } from '../lib/queries';
          ...
          <DialogContent>
            ...
            <div className="space-y-4 py-4">
              ...
              <div>
                ...
                <Select value={newTodo.recipientId} onValueChange={(value) => setNewTodo({ ...newTodo, recipientId: value })}>
                  <SelectTrigger>
            ...
            <CardContent>
              <div className="space-y-3">
                ...
                {activeTodos.map((todo) => {
                  const recipient = recipients?.find(r => r.id === todo.recipientId);
                  return (
                    <div key={todo.id} className="flex items-start gap-3 p-3 border rounded-lg hover:bg-gray-50">
                      <Checkbox
                      ...
                      <div id={String(todo.id)} className="flex-1">
                        ...
                        <div className="flex items-center gap-4 mt-2 text-xs text-gray-500">
                          ...
                          <span>{format(todo.dueDate, 'MMM d, yyyy h:mm a')}</span>
                        </div>
                      </div>
                    </div>
                  );
              ...
              <CardContent>
                <div className="space-y-3">
                  {completedTodos.map((todo) => {
                    const recipient = recipients?.find(r => r.id === todo.recipientId);
                    return (
                      <div key={todo.id} className="flex items-start gap-3 p-3 border rounded-lg bg-gray-50 opacity-75">
                        <Checkbox
                          checked={todo.completed}
                          onCheckedChange={() => handleToggleTodo(todo.id)}
                          className="mt-1"
